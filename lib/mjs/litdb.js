...
class Inspect {
  ...
  static dumpTable(rows) {
    ...
    let rowWidth = Object.keys(sizes).map((k) => sizes[k]).reduce((p, c) => p + c, 0) + sizesLen * 2 + (sizesLen + 1);
    let sb = [];
...
class Schema {
  ...
  sqlIndexDefinition(table2, col) {
    ...
    return `CREATE ${unique} ${name} ON ${this.quoteTable(table2.name)} (${this.quoteColumn(col)})`;
  }
  sqlForeignKeyDefinition(table2, col) {
    ...
    const refKeys = Array.isArray(ref.table) ? Array.isArray(ref.table[1]) ? ref.table[1].map((x) => $.quoteColumn(x)).join(",") : $.quoteColumn(ref.table[1]) : refMeta.columns.filter((x) => x.primaryKey).map((x) => $.quoteColumn(x)).join(",");
  ...
  sqlColumnDefinition(col) {
    ...
    let sb = `${this.quoteColumn(col)} ${type2}`;
    if (col.primaryKey) {
      sb += " PRIMARY KEY";
    }
  ...
  toResult(o, cls) {
    if (cls && o != null && IS.obj(o)) {
      ...
      M.props.filter((p) => p.column).forEach((p) => {
        const val = o[p.column.alias ?? (hasStrategy ? dialect.strategy.columnName(p.name) : p.name)];
        const conv = schema.converters[p.column.type];
        to[p.name] = conv ? conv.fromDb(val) : val;
      });
      return to;
    }
...
class SelectQuery extends WhereQuery {
  ...
  buildSelect() {
    const sqlSelect = this._select.length > 0 ? this._select.join(", ") : this.meta.columns.map((x) => this.quoteColumn(x)).join(", ");
...
class SqlBuilderBase {
  ...
  build(refs) {
    ...
    const sqls = [];
    const queryRefs = [];
    for (const table2 of this.tables) {
      const useRef = refs.find((x) => x.$ref.cls === table2) ?? this.$.ref(table2);
...
class DbConnection {
  ...
  prepare(str, ...params) {
    ...
    if (IS.tpl(str)) {
      let stmt = C.prepare(str, ...params);
      return [stmt, params, undefined];
    } else if (IS.obj(str)) {
      if ("build" in str) {
        let query = str.build();
        let stmt = C.prepare(query.sql);
...
class SyncDbConnection {
  ...
  insert(row, options) {
    ...
    if (options?.onlyProps || options?.onlyWithValues) {
      ...
      let stmt = this.connection.prepareSync(S.insert(cls, onlyOptions));
      const dbRow = S.toDbObject(row, onlyOptions);
      return stmt.execSync(dbRow);
    } else {
      let stmt = this.connection.prepareSync(S.insert(cls));
  ...
  update(row, options) {
    ...
    if (options?.onlyProps || options?.onlyWithValues) {
      ...
      const onlyOptions = { onlyProps };
      let stmt = this.connection.prepareSync(S.update(cls, onlyOptions));
      const dbRow = S.toDbObject(row, onlyOptions);
      return stmt.execSync(dbRow);
    } else {
      let stmt = this.connection.prepareSync(S.update(cls));
      const dbRow = S.toDbObject(row);
      return stmt.execSync(dbRow);
    }
  }
  ...
  prepareSync(str, ...params) {
    const C = this.connection;
    if (IS.tpl(str)) {
      let stmt = C.prepareSync(str, ...params);
      return [stmt, params, undefined];
    } else if (IS.obj(str)) {
      if ("build" in str) {
        let query = str.build();
        let stmt = C.prepareSync(query.sql);
        return [stmt, query.params ?? {}, query.into];
      } else if ("sql" in str) {
        let sql = str.sql;
        let params2 = str.params ?? {};
        let stmt = C.prepareSync(sql);
        return [stmt, params2, str.into];
      }
    }
    throw new Error(`Invalid argument: ${toStr(str)}`);
  ...
  all(strings, ...params) {
    ...
    if (into) {
      ...
      return IS.arr(p) ? use.allSync(...p) : use.allSync(p);
    } else {
  ...
  one(strings, ...params) {
    ...
    if (into) {
      const use = stmt.as(into);
      return IS.arr(p) ? use.oneSync(...p) : use.oneSync(p);
    } else {
  ...
  column(strings, ...params) {
    ...
    return IS.arr(p) ? stmt.arraysSync(...p).map((x) => x[0]) : stmt.arraysSync(p).map((x) => x[0]);
  }
  value(strings, ...params) {
    const [stmt, p, into] = this.prepareSync(strings, ...params);
  ...
  arrays(strings, ...params) {
    const [stmt, p] = this.prepareSync(strings, ...params);
    return IS.arr(p) ? stmt.arraysSync(...p) : stmt.arraysSync(p);
  }
  array(strings, ...params) {
    const [stmt, p] = this.prepareSync(strings, ...params);
    return IS.arr(p) ? stmt.arraySync(...p) : stmt.arraySync(p);
  }
  exec(strings, ...params) {
    const [stmt, p] = this.prepareSync(strings, ...params);
...
class DialectBase {
  ...
  quoteTable(table2) {
    return IS.str(table2) ? isQuoted(table2) ? table2 : this.quote(this.strategy.tableName(table2)) : this.quote(table2.alias ?? this.strategy.tableName(table2.name));
  }
  quoteColumn(column2) {
    return IS.str(column2) ? isQuoted(column2) ? column2 : this.quote(this.strategy.columnName(column2)) : this.quote(column2.alias ?? this.strategy.columnName(column2.name));
  }
...
class SqliteDialect extends DialectBase {
}
...
class MySqlDialect extends DialectBase {
  quote(name) {
    return isQuoted(name) ? name : "`" + name + "`";
  }
  sqlLimit(offset, limit) {
...
class MySqlSchema extends Schema {
  ...
  sqlIndexDefinition(table2, col) {
    ...
    return `CREATE ${unique} ${name} ON ${this.quoteTable(table2)} (${this.quoteColumn(col)}${indexSize})`;
  }
  sqlColumnDefinition(col) {
    let type2 = this.dataType(col);
    let sb = `${this.quoteColumn(col)} ${type2}`;
...
class PostgreSqlDialect extends DialectBase {
  sqlLimit(offset, limit) {
    if (offset == null && limit == null)
      throw new Error(`Invalid argument sqlLimit(${offset}, ${limit})`);
